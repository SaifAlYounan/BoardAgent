-- Browser-key changes revoke existing sessions/grants and pending approvals atomically.
create table public.key_lifecycle_browser_effects (
  operation_id uuid not null references public.key_lifecycle_operations(id) on delete restrict,
  target_type text not null check (target_type in ('session','authorization_request','action_stage','onboarding_stage')),
  target_id uuid not null,
  session_id uuid references public.auth_sessions(id) on delete restrict,
  authorization_request_id uuid references public.oauth_authorization_requests(id) on delete restrict,
  action_stage_id uuid references public.action_stages(id) on delete restrict,
  onboarding_stage_id uuid references public.onboarding_browser_stages(id) on delete restrict,
  ...
  check (num_nonnulls(session_id,authorization_request_id,action_stage_id,onboarding_stage_id)=1
    and case target_type when 'session' then session_id=target_id
      when 'authorization_request' then authorization_request_id=target_id
      when 'action_stage' then action_stage_id=target_id when 'onboarding_stage' then onboarding_stage_id=target_id else false end is true)
);
alter table public.key_lifecycle_browser_effects owner to boardagent_migrator;
alter table public.key_lifecycle_browser_effects enable row level security;
alter table public.key_lifecycle_browser_effects force row level security;
revoke all on public.key_lifecycle_browser_effects from public;
grant select on public.key_lifecycle_browser_effects to boardagent_backup;
create policy boardagent_key_browser_operator_read on public.key_lifecycle_browser_effects
  for select to boardagent_migrator using (current_setting('boardagent.transaction_scope',true)='bootstrap');
create policy boardagent_key_browser_operator_insert on public.key_lifecycle_browser_effects
  for insert to boardagent_migrator with check (current_setting('boardagent.transaction_scope',true)='bootstrap');
create policy boardagent_key_browser_backup_read on public.key_lifecycle_browser_effects
  for select to boardagent_backup using (true);
create trigger boardagent_immutable before update or delete on public.key_lifecycle_browser_effects
  for each row execute function public.boardagent_reject_evidence_mutation();

create function public.boardagent_guard_key_browser_effect()
...
begin
  ...
  if current_setting('boardagent.transaction_scope',true) is distinct from 'bootstrap'
    ...
    or op.details->>'purpose'<>'browser_session' then
...
$$;
alter function public.boardagent_guard_key_browser_effect() owner to boardagent_migrator;
revoke all on function public.boardagent_guard_key_browser_effect() from public;
create trigger boardagent_key_browser_guard before insert on public.key_lifecycle_browser_effects
  for each row execute function public.boardagent_guard_key_browser_effect();

grant update(state) on public.auth_sessions to boardagent_migrator;
create policy boardagent_key_browser_update on public.auth_sessions for update to boardagent_migrator
  using (current_setting('boardagent.transaction_scope',true)='bootstrap' and exists(
    select 1 from public.key_lifecycle_operations op where op.organization_id=auth_sessions.organization_id
      and op.authorization_transaction_id=pg_current_xact_id()
      and op.authorization_server_start=pg_postmaster_start_time() and op.details->>'purpose'='browser_session'))
  with check (current_setting('boardagent.transaction_scope',true)='bootstrap' and exists(
    select 1 from public.key_lifecycle_browser_effects effect join public.key_lifecycle_operations op on op.id=effect.operation_id
      where effect.target_type='session' and effect.target_id=auth_sessions.id
        and op.authorization_transaction_id=pg_current_xact_id()
        and op.authorization_server_start=pg_postmaster_start_time()));

grant update(request_state) on public.oauth_authorization_requests to boardagent_migrator;
create policy boardagent_key_browser_update on public.oauth_authorization_requests for update to boardagent_migrator
  using (current_setting('boardagent.transaction_scope',true)='bootstrap' and exists(
    select 1 from public.key_lifecycle_operations op where op.organization_id=oauth_authorization_requests.organization_id
      and op.authorization_transaction_id=pg_current_xact_id()
      and op.authorization_server_start=pg_postmaster_start_time() and op.details->>'purpose'='browser_session'))
  with check (current_setting('boardagent.transaction_scope',true)='bootstrap' and exists(
    select 1 from public.key_lifecycle_browser_effects effect join public.key_lifecycle_operations op on op.id=effect.operation_id
      where effect.target_type='authorization_request' and effect.target_id=oauth_authorization_requests.id
        and op.authorization_transaction_id=pg_current_xact_id()
        and op.authorization_server_start=pg_postmaster_start_time()));

grant update(state,cancelled_at) on public.action_stages to boardagent_migrator;
create policy boardagent_key_browser_update on public.action_stages for update to boardagent_migrator
  using (current_setting('boardagent.transaction_scope',true)='bootstrap' and exists(
    select 1 from public.key_lifecycle_operations op where op.organization_id=action_stages.organization_id
      and op.authorization_transaction_id=pg_current_xact_id()
      and op.authorization_server_start=pg_postmaster_start_time() and op.details->>'purpose'='browser_session'))
  with check (current_setting('boardagent.transaction_scope',true)='bootstrap' and exists(
    select 1 from public.key_lifecycle_browser_effects effect join public.key_lifecycle_operations op on op.id=effect.operation_id
      where effect.target_type='action_stage' and effect.target_id=action_stages.id
        and op.authorization_transaction_id=pg_current_xact_id()
        and op.authorization_server_start=pg_postmaster_start_time()));

create function public.boardagent_key_browser_effects_match(candidate_operation_id uuid)
...
begin
  ...
  for item in select * from (values ('session','browser_sessions'),
    ...
    if op.details->>'purpose'='browser_session' then
...
$$;
alter function public.boardagent_key_browser_effects_match(uuid) owner to boardagent_migrator;
revoke all on function public.boardagent_key_browser_effects_match(uuid) from public;
...
begin
  ...
  if k.purpose='browser_session' then
...
begin
  ...
  if current_setting('boardagent.transaction_scope',true) is distinct from 'bootstrap'
    or op.id is null or op.authorization_transaction_id<>pg_current_xact_id()
    or op.authorization_server_start<>pg_postmaster_start_time()
    or not (op.details->>'purpose'='browser_session' or (op.details->>'purpose'='oauth_signing' and op.details->>'operation'='mark_compromised')) then
  ...
  if family.id is null or family.organization_id<>op.organization_id or family.state<>'active'
    or (op.details->>'purpose'<>'browser_session' and not exists(select 1 from public.access_token_records a where a.refresh_family_id=family.id and a.signing_key_id=op.key_id)) then
...
alter policy boardagent_key_family_revocation on public.refresh_families to boardagent_migrator
  using (current_setting('boardagent.transaction_scope',true)='bootstrap' and exists(
    select 1 from public.key_lifecycle_operations op
      where op.authorization_transaction_id=pg_current_xact_id()
        ...
        and (op.details->>'purpose'='browser_session' or (op.details->>'purpose'='oauth_signing' and op.details->>'operation'='mark_compromised'
...
begin
  select stored.* into op from public.key_lifecycle_operations stored where stored.id=candidate_operation_id;
  if op.id is null or public.boardagent_key_lifecycle_families_match(op.id) is distinct from true
...
begin
  ...
  begin
    ...
    if jsonb_typeof(request)<>'object' or not request ?& fields or request-fields<>'{}'::jsonb
      ...
      or request->>'purpose' not in ('evidence_signing','oauth_signing','browser_session')
  ...
  algorithm:=case when request->>'purpose'='oauth_signing' then 'ES256' when request->>'purpose'='browser_session' then 'HMAC-SHA256' else 'EdDSA' end;
  ...
  if request->>'operation'='replace' then
    begin
      ...
      if jsonb_typeof(replacement) is distinct from 'object'
        ...
        or (request->>'purpose'='browser_session' and new_public is distinct from 'null'::jsonb)
        or (request->>'purpose'<>'browser_session' and jsonb_typeof(new_public) is distinct from 'object')
      ...
      if request->>'purpose'='browser_session' then
        fingerprint:=null;
...
begin
  ...
  if stored.details->>'purpose'='browser_session' or (stored.details->>'purpose'='oauth_signing' and stored.details->>'operation'='mark_compromised') then
    insert into public.key_lifecycle_affected_families(operation_id,family_id)
      ...
      where f.organization_id=stored.organization_id and f.state='active'
        and (stored.details->>'purpose'='browser_session' or exists(select 1 from public.access_token_records token where token.refresh_family_id=f.id and token.signing_key_id=stored.key_id))
      order by f.id;
  ...
  if stored.details->>'purpose'='browser_session' then
    insert into public.key_lifecycle_browser_effects(operation_id,target_type,target_id,session_id)
      select stored.id,'session',t.id,t.id from public.auth_sessions t
        where t.organization_id=stored.organization_id and t.state in ('anonymous','authenticated') order by t.id;
    ...
    insert into public.key_lifecycle_browser_effects(operation_id,target_type,target_id,authorization_request_id)
      select stored.id,'authorization_request',t.id,t.id from public.oauth_authorization_requests t
        where t.organization_id=stored.organization_id and t.request_state in ('pending','approved') order by t.id;
    ...
    insert into public.key_lifecycle_browser_effects(operation_id,target_type,target_id,action_stage_id)
      select stored.id,'action_stage',t.id,t.id from public.action_stages t
        where t.organization_id=stored.organization_id and t.state='active' order by t.id;
...
grant select,update(state) on public.onboarding_browser_stages to boardagent_migrator;
create policy boardagent_key_browser_read on public.onboarding_browser_stages for select to boardagent_migrator
  using (current_setting('boardagent.transaction_scope',true)='bootstrap');
create policy boardagent_key_browser_update on public.onboarding_browser_stages for update to boardagent_migrator
  using (current_setting('boardagent.transaction_scope',true)='bootstrap' and exists(
    select 1 from public.key_lifecycle_operations op where op.organization_id=onboarding_browser_stages.organization_id
      and op.authorization_transaction_id=pg_current_xact_id()
      and op.authorization_server_start=pg_postmaster_start_time() and op.details->>'purpose'='browser_session'))
  with check (current_setting('boardagent.transaction_scope',true)='bootstrap' and exists(
    select 1 from public.key_lifecycle_browser_effects effect join public.key_lifecycle_operations op on op.id=effect.operation_id
      where effect.target_type='onboarding_stage' and effect.target_id=onboarding_browser_stages.id
        and op.authorization_transaction_id=pg_current_xact_id()
        and op.authorization_server_start=pg_postmaster_start_time()));
  ...
  if old.request_state='approved' and new.request_state='expired'
    ...
    if exists(select 1 from public.key_lifecycle_browser_effects effect
      ...
      where effect.target_type='authorization_request' and effect.authorization_request_id=old.id
        and op.authorization_transaction_id=pg_current_xact_id()
        and op.authorization_server_start=pg_postmaster_start_time()
        and op.organization_id=old.organization_id and op.details->>'purpose'='browser_session') then
      return new;
    end if;
  end if;
...
begin
  ...
  for work in select * from (values
    ...
    ('browser_action_stages',$query$select jsonb_build_object('kind','action_stage','row',to_jsonb(s)) as value from public.action_stages s where s.organization_id=$1 and $3='browser_session' and s.state='active' union all select jsonb_build_object('kind','onboarding_stage','row',to_jsonb(t)) as value from public.onboarding_browser_stages t where t.organization_id=$1 and $3='browser_session' and t.state='active'$query$),
  ...
  if tg_op='UPDATE' and current_user='boardagent_migrator'
    ...
    select exists(select 1 from public.key_lifecycle_browser_effects effect
      ...
      where effect.target_type='onboarding_stage' and effect.onboarding_stage_id=old.id
        and op.authorization_transaction_id=pg_current_xact_id()
        and op.authorization_server_start=pg_postmaster_start_time()
        and op.organization_id=old.organization_id and op.details->>'purpose'='browser_session') into maintenance;
  end if;
