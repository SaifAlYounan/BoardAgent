-- BoardAgent Phase 4 / group 81: membership-generation feed reconciliation and
-- read-only consistency evidence. Reconciliation may only append a safe removal
-- tombstone for a pending projection whose board/member entitlement is provably stale.
-- Structural/hash drift is reported by the consistency lane and is never auto-repaired.

grant update(state,resolved_at) on public.pending_action_feed to boardagent_migrator;
grant insert on public.feed_tombstones to boardagent_migrator;

create policy boardagent_migrator_feed_reconcile_update
  on public.pending_action_feed for update to boardagent_migrator
...
create policy boardagent_migrator_feed_reconcile_tombstone_insert
  on public.feed_tombstones for insert to boardagent_migrator
  with check (current_setting('boardagent.transaction_scope',true)='worker');

create function public.boardagent_feed_reconcile_candidates(
...
create function public.boardagent_commit_feed_revocation(
  candidate_organization_id uuid,
  candidate_board_id uuid,
  candidate_member_id uuid,
...
create function public.boardagent_feed_consistency_payloads(
  candidate_organization_id uuid,
  candidate_after_id uuid,
  candidate_limit integer
)
returns table(
  feed_id uuid,
...
begin
  if current_setting('boardagent.transaction_scope',true) is distinct from 'worker'
     or candidate_organization_id is null
     or not public.boardagent_is_uuid_v7(candidate_organization_id)
     or (candidate_after_id is not null
         and not public.boardagent_is_uuid_v7(candidate_after_id))
     or candidate_limit is null
     or candidate_limit not between 1 and 1000 then
    raise exception 'feed consistency payload scope or limit is invalid' using errcode='22023';
  end if;
  if not exists (
  ...
  return query
    select feed.id,feed.board_id,feed.member_id,feed.action_type,feed.object_type,
           feed.object_id,feed.object_version,
           feed.entitlement_generation,feed.feed_sequence,feed.canonical_payload,
           encode(feed.payload_sha256,'hex')
      from public.pending_action_feed as feed
     where feed.organization_id=candidate_organization_id
       and (candidate_after_id is null or feed.id>candidate_after_id)
     order by feed.id
     limit candidate_limit;
end
$$;

create function public.boardagent_feed_consistency_relations(candidate_organization_id uuid)
returns jsonb
language plpgsql
stable
security definer
       ...
       or not exists (
         select 1 from public.audit_events as audit
          where audit.id=tombstone.audit_event_id
            ...
            and audit.object_type=tombstone.object_type
            and audit.object_id=tombstone.object_id
       )
     );
...
alter function public.boardagent_commit_feed_revocation(uuid,uuid,uuid,uuid,uuid,bytea)
  owner to boardagent_migrator;
alter function public.boardagent_feed_consistency_payloads(uuid,uuid,integer)
  owner to boardagent_migrator;
alter function public.boardagent_feed_consistency_relations(uuid)
  owner to boardagent_migrator;

revoke all on function public.boardagent_feed_reconcile_candidates(uuid,uuid,uuid,integer)
  from public;
revoke all on function public.boardagent_commit_feed_revocation(uuid,uuid,uuid,uuid,uuid,bytea)
  from public;
revoke all on function public.boardagent_feed_consistency_payloads(uuid,uuid,integer)
  from public;
revoke all on function public.boardagent_feed_consistency_relations(uuid)
  from public;

grant execute on function public.boardagent_feed_reconcile_candidates(uuid,uuid,uuid,integer)
  to boardagent_worker;
grant execute on function public.boardagent_commit_feed_revocation(uuid,uuid,uuid,uuid,uuid,bytea)
  to boardagent_worker;
grant execute on function public.boardagent_feed_consistency_payloads(uuid,uuid,integer)
  to boardagent_worker;
grant execute on function public.boardagent_feed_consistency_relations(uuid)
  to boardagent_worker;
