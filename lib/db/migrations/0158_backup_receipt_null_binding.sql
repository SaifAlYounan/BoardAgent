-- Bind backup/restore evidence even when JSON fields are missing or null.
-- Historical receipt bytes remain immutable; health exposes existing bad bindings.
-- Preserve existing state constraints and recovery evidence. Failed restores may
-- retain a null verifiedAt only when their row has no verification timestamp;
-- they remain non-healthy, and no runtime writer records them as verified.
...
begin
  ...
  if jsonb_typeof(manifest) is distinct from 'object' or exists (
    ...
    ] || case when new.receipt_kind='restore' then
      array['sourceBackupReceiptId','sourceBackupManifestSha256'] else array[]::text[] end
    || case when new.verified_restore_at is not null then
      array['verifiedAt'] else array[]::text[] end) as field(name)
    where jsonb_typeof(manifest->field.name) is distinct from 'string'
  ) then
    raise exception 'backup or restore receipt binding fields must be JSON strings'
      using errcode='23514';
  end if;
  ...
  if new.organization_id<>instance_row.organization_id
     ...
     or manifest->>'receiptId' is distinct from new.id::text
     or manifest->>'instanceId' is distinct from instance_row.instance_id::text
     or manifest->>'organizationId' is distinct from new.organization_id::text
     or manifest->>'snapshotLsn' is distinct from new.snapshot_lsn::text
     or manifest->>'snapshotAt' is distinct from to_char(
       new.snapshot_at at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'
     )
     or manifest->>'contentSetSha256' is distinct from encode(new.content_set_sha256,'hex')
     or manifest->>'encryptionKeyId' is distinct from new.encryption_key_id::text
  ...
  if new.receipt_kind='backup' then
    if manifest->>'schemaVersion' is distinct from new.schema_version
       ...
       or manifest->>'receiptKind' is distinct from 'backup' then
    ...
    if source_receipt.receipt_kind<>'backup'
       ...
       or manifest->>'receiptKind' is distinct from 'restore'
       or manifest->>'sourceBackupReceiptId' is distinct from source_receipt.id::text
       or manifest->>'sourceBackupManifestSha256' is distinct from encode(source_receipt.manifest_sha256,'hex')
       or manifest->>'verifiedAt' is distinct from to_char(
...
begin
  ...
  select instance.instance_id into expected_instance_id
  ...
  loop
    ...
    if exists (
      select 1 from unnest(array[
        'schemaVersion','receiptKind','receiptId','instanceId','organizationId',
        'snapshotLsn','snapshotAt','contentSetSha256','encryptionKeyId'
      ] || case when receipt.receipt_kind='restore' then
        array['sourceBackupReceiptId','sourceBackupManifestSha256','verifiedAt']
        else array[]::text[] end) as field(name)
      where jsonb_typeof(manifest->field.name) is distinct from 'string'
    ) then
      manifest_schema_mismatches := manifest_schema_mismatches+1;
    end if;

    if manifest->>'receiptId' is distinct from receipt.id::text
       or manifest->>'instanceId' is distinct from expected_instance_id::text
       or manifest->>'organizationId' is distinct from receipt.organization_id::text
       or manifest->>'snapshotLsn' is distinct from receipt.snapshot_lsn::text
       or manifest->>'snapshotAt' is distinct from to_char(
         receipt.snapshot_at at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'
       )
       or manifest->>'contentSetSha256' is distinct from encode(receipt.content_set_sha256,'hex')
       or manifest->>'encryptionKeyId' is distinct from receipt.encryption_key_id::text then
    ...
    if receipt.receipt_kind='backup' then
      ...
      if receipt.schema_version not in ('boardagent.restore-receipt.v1','boardagent.restore-receipt.v2')
         ...
         or manifest->>'schemaVersion' is distinct from receipt.schema_version
         or manifest->>'receiptKind' is distinct from 'restore' then
        manifest_schema_mismatches := manifest_schema_mismatches+1;
      end if;
