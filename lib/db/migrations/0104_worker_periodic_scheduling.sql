-- Recurring work is derived from database time and live instance/board records.
-- No caller-controlled organization, subject, payload, watermark, cadence or job type.
create index boardagent_jobs_pending_schedule
  on public.jobs(organization_id,job_type,subject_id)
  where state in ('queued','retry','leased');

create function public.boardagent_schedule_periodic_jobs()
returns integer
language plpgsql
volatile
security definer
...
begin
  ...
  for target in
    with organization_types(job_type,period_seconds,priority) as (values
      ...
      select candidate.*,
        'worker-periodic:'||candidate.job_type||':'||coalesce(candidate.board_id::text,'organization')||
