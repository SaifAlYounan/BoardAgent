-- MR-MEC-001 / SR-027/039/043/097: complete the existing confirmed seat-role
-- change with an optional company-admin-only chair attribute. No new callable,
-- event, delegated power, or implicit profile mutation. Keep exact before/after
-- consent, protected role targets, revocation, and historical membership versions.
-- Based on the final authority definition0101 and lifecycle definitions0091;
-- retain the SQL98 trusted search path and all historical migration bytes.
...
begin
  ...
  if change-(fields||case when action='invite' then array['reason'] when action='change_seat' then array['is_chair'] else array[]::text[] end)<>'{}'::jsonb or not(change ?& fields)
    or (change ? 'is_chair' and jsonb_typeof(change->'is_chair') is distinct from 'boolean')
  ...
  -- Existing delegated role/weight authority does not appoint or remove a chair.
  if change ? 'is_chair' then
    raise exception 'chair assignment requires a company administrator' using errcode='42501';
  end if;
...
begin
  ...
  if operation='change_seat' then
    if change - array['operation','member_id','board_id','reason','seat_role','voting_weight','is_secretary','is_chair']<>'{}'::jsonb
  ...
  loop
    ...
    next_chair := case when operation='change_seat' and change ? 'is_chair'
      then (change->>'is_chair')::boolean
      else seat.is_chair and next_role='voting_member' end;
    if next_chair and next_role<>'voting_member' then
      raise exception 'only voting members may chair' using errcode='23514';
    end if;
    -- The original unique index covers only open-ended active seats. Also reject
    -- overlapping time-limited chairs under the existing organization/board locks.
    -- Expired history does not block a later appointment. Reactivating an ended
    -- seat starts a fresh unbounded window, just as lifecycle finalization does.
    if next_chair and next_state='active' and exists (
      select 1 from public.board_memberships other
       where other.organization_id=org and other.board_id=seat.board_id
         and other.id<>seat.id and other.state='active' and other.is_chair
         and (other.active_until is null or other.active_until>transaction_timestamp())
         and case when seat.state<>'ended' and seat.active_until<=transaction_timestamp() then false
           else tstzrange(other.active_from,other.active_until,'[)') && tstzrange(
           case when seat.state='ended' then transaction_timestamp()
             else greatest(seat.active_from,transaction_timestamp()) end,
           case when seat.state='ended' then null else seat.active_until end,'[)') end
    ) then
      raise exception 'board already has a chair in the active appointment window' using errcode='23514';
    end if;
...
begin
  ...
  for next_seat in select value from jsonb_array_elements(snapshot->'seatsAfter') loop
    ...
    insert into public.membership_versions(id,organization_id,board_id,member_id,membership_id,version,
      seat_role,is_secretary,is_chair,voting_weight,authority_snapshot,snapshot_sha256,change_reason,actor_member_id,consent_record_id,audit_event_id)
     values((version->>'id')::uuid,org,(next_seat->>'boardId')::uuid,target_member_id,(next_seat->>'membershipId')::uuid,next_version,
      next_seat->>'seatRole',(next_seat->>'isSecretary')::boolean,(next_seat->>'isChair')::boolean,(next_seat->>'votingWeight')::bigint,
