-- First real client login (14 September 2026, Claude Code 2.1.270 against the private
-- beta instance): the client registers a loopback callback as http://localhost:<port>/callback,
-- which the contract now accepts (lib/contracts/src/oauth-redirect.ts, RFC 8252 §7.3) but
-- this registration authority still refused, so registration failed with a generic error.
-- The function below is SQL0166's boardagent_register_oauth_client verbatim with one
-- more accepted loopback form; ownership and grants are unchanged. Nothing else moves.
create or replace function public.boardagent_register_oauth_client(
...
begin
  ...
  if candidate_internal_id is null
     or not public.boardagent_is_uuid_v7(candidate_internal_id)
     or candidate_protocol_id_kind is null
     ...
     or candidate_max_clients is null
  ...
  if candidate_safe_metadata is null
     ...
     or candidate_safe_metadata->'schemaVersion' is distinct from '1'::jsonb
     or candidate_safe_metadata->>'registrationMethod' is distinct from expected_registration_method
     or jsonb_typeof(candidate_safe_metadata->'name') is distinct from 'string'
  ...
  if candidate_redirect_uris is null
     or candidate_redirect_uri_sha256 is null
     or redirect_count not between 1 and 10
       ...
       select 1
        ...
        where redirect.uri is null
           ...
           or not (
             ...
             or redirect.uri ~ '^http://localhost:[0-9]+(?:/|$)'
  ...
  if candidate_scopes is null
     or cardinality(candidate_scopes) not between 1 and 128
     or exists (
