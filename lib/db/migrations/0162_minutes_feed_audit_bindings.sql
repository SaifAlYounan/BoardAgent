-- Recognize only existing minutes signature notice evidence through its exact
-- permanent package and configured recipient. Preserve historical package states,
-- all canonical rows, existing question/onboarding mappings and routine grants.

create function public.boardagent_minutes_feed_audit_binding(
  candidate_feed public.pending_action_feed,
  candidate_audit public.audit_events
)
returns boolean
language plpgsql
stable
...
declare
  ...
  audit_payload jsonb;
begin
  if (candidate_feed.object_type='minutes'
      and candidate_feed.action_type in ('minutes_signature_required','minutes_resign_required')
      and candidate_feed.notice_id is not null
      and candidate_audit.id=candidate_feed.audit_event_id
      and candidate_audit.organization_id=candidate_feed.organization_id
      and candidate_audit.board_id=candidate_feed.board_id
      and candidate_audit.object_type='minutes_signature_package'
      and candidate_audit.event_type='notice_delivered') is distinct from true then
    return false;
  end if;
  begin
    feed_payload:=convert_from(candidate_feed.canonical_payload,'UTF8')::jsonb;
    audit_payload:=convert_from(candidate_audit.canonical_payload,'UTF8')::jsonb;
  exception when character_not_in_repertoire or invalid_text_representation
      or sqlstate '22003' or sqlstate '22P05' then
    -- JSON numeric overflow and unsupported escaped Unicode also refuse this binding.
    return false;
  end;
  if (jsonb_typeof(feed_payload)='object'
      and jsonb_typeof(feed_payload->'safeRefs')='object'
      and jsonb_typeof(audit_payload)='object'
      and jsonb_typeof(audit_payload->'details')='object'
      and audit_payload->'details'->>'recipientMemberId'=candidate_feed.member_id::text
      and audit_payload->'details'->>'noticeType'=candidate_feed.action_type
      and audit_payload->'details'->>'meaning'='committed_recipient_feed_handoff') is distinct from true then
    return false;
  end if;
  return exists (
    select 1 from public.minutes_signature_packages package
    join public.minutes minutes
    ...
    join public.minutes_signature_requirements requirement
      on requirement.package_id=package.id and requirement.member_id=candidate_feed.member_id
    where package.id=candidate_audit.object_id
      and package.organization_id=candidate_feed.organization_id
      and package.board_id=candidate_feed.board_id
      and package.minutes_id=candidate_feed.object_id
      and package.id::text=feed_payload->'safeRefs'->>'signaturePackageId'
      and encode(package.package_sha256,'hex')=feed_payload->'safeRefs'->>'packageSha256'
  );
end
$$;
alter function public.boardagent_minutes_feed_audit_binding(public.pending_action_feed,public.audit_events)
  owner to boardagent_migrator;
revoke all on function public.boardagent_minutes_feed_audit_binding(public.pending_action_feed,public.audit_events)
  from public;
...
begin
  ...
  return query
    select feed.id,
          ...
          or public.boardagent_minutes_feed_audit_binding(feed,audit))
           )
    ...
    select 1 from public.audit_events as audit
     where audit.id=target.audit_event_id
       ...
       and audit.object_id=target.object_id)
          or exists (select 1 from public.pending_action_feed source_feed
            where source_feed.id=target.id
              and source_feed.organization_id=target.organization_id
              and source_feed.board_id=target.board_id
              and source_feed.member_id=target.member_id
              and public.boardagent_minutes_feed_audit_binding(source_feed,audit)))
  ) then
         ...
         select 1 from public.audit_events as audit
          where audit.id=tombstone.audit_event_id
            ...
            and ((audit.object_type=tombstone.object_type and audit.object_id=tombstone.object_id)
              ...
              or (tombstone.object_type='minutes' and tombstone.reason_class='revoked'
                and exists(select 1 from public.pending_action_feed removed
                  where removed.id=tombstone.removed_feed_id
                    and removed.organization_id=tombstone.organization_id
                    and removed.board_id=tombstone.board_id
                    ...
                    and removed.object_type=tombstone.object_type
                    and removed.object_id=tombstone.object_id
                    and removed.feed_sequence=tombstone.feed_sequence
                    and removed.state='superseded'
                    and removed.audit_event_id=tombstone.audit_event_id
                    and public.boardagent_minutes_feed_audit_binding(removed,audit)))
