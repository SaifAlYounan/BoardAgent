...
const origin = "https://boardagent.test",
  ...
  token = "synthetic-vote-tool-native-token";
const timeout = 10000;
type Route = "visible" | "absent";
...
it("delivers exact visible and absent vote tools through native HTTPS SDK and real empty collector settlement", async () => {
  await withMigratedDatabase("vote_tool_native", async (pool) => {
    ...
    // Ordinary constrained draft storage only: no ballots, outcome, certificate,
    // public governance creation/closing or worker is represented.
    const vote = await seedVoteProjectionFixture(pool, actor);
    const voteIds = { visible: vote.voteId, absent: testId(225003) };
    const resourceUris = {
      visible: `board://${actor.boardId}/votes/${vote.voteId}`,
      absent: null
    };
    const oracle = await withRequestTransaction(
      ...
      async (connection) => {
        ...
        >;
        for (const route of ["visible", "absent"] as const) {
          const found = await connection.query<{ view: JsonValue }>(ORIGINAL_VOTE_TOOL_SQL, [
            voteIds[route],
            actor.memberId
          ]);
          const measured = await connection.query<VoteToolProjectionMetadata>(
            VOTE_TOOL_PREFLIGHT_SQL,
            [voteIds[route], actor.memberId]
          );
          expect(found.rows).toHaveLength(route === "visible" ? 1 : 0);
          expect(measured.rows).toHaveLength(route === "visible" ? 1 : 0);
          const view = found.rows[0]?.view ?? null;
          if (route === "visible") {
            if (!object(view)) throw new Error("expected original fourteen-field vote view");
            expect(Object.keys(view)).toHaveLength(14);
            expect(view["board_id"]).toBe(actor.boardId);
            expect(view["state"]).toBe("draft");
            expect(view["my_ballots"]).toEqual([]);
            expect(view["outcome"]).toBeNull();
            const resolution = view["resolution"],
              decision = view["decision_package"];
            if (!object(resolution) || !object(decision))
              throw new Error("missing original joined fields");
            expect(Object.keys(resolution)).toHaveLength(4);
            expect(Object.keys(decision)).toHaveLength(12);
          } else expect(view).toBeNull();
          const result = {
            schema_version: "boardagent.tool-result.v1",
            tool: "get_vote",
            status: "ok",
            reference: voteIds[route],
            resource_uri: resourceUris[route],
            data: { vote: view }
          } as JsonValue;
          expected[route] = {
            view,
            ...
            units: measured.rows[0] ? voteToolProjectionPlan(measured.rows[0]).units : 0
          };
        }
        expect(expected.visible.units).toBeGreaterThan(0);
        expect(expected.absent.units).toBe(0);
        return expected;
      },
    ...
    const entered = { visible: gate(), absent: gate() },
      releaseSettlement = { visible: gate(), absent: gate() };
    ...
    const wire = { visible: 0, absent: 0 },
      media = { visible: "", absent: "" };
    ...
    const server = createServer(tls, (request, response) => {
      const route = request.headers["x-vote-tool-fixture"];
      if (route === "visible" || route === "absent") {
    ...
    // Plain get_vote returns no prepared delivery handle. Delegate
    ...
    const settleSpy = vi
      ...
      .mockImplementation(function (
        ...
        const route = (["visible", "absent"] as const).find(
    ...
    try {
      ...
      const service: BoardAgentSurfaceService = {
        ...
        executeRead: async (principal, tool, input) => {
          if (tool !== "get_vote" || !object(input))
            throw new Error("unexpected native tool request");
          const route: Route = input["vote_id"] === voteIds.visible ? "visible" : "absent";
          if (input["vote_id"] !== voteIds[route]) throw new Error("unexpected vote selector");
      ...
      const transportFetch: typeof fetch = async (input, init) => {
        ...
        if (object(parsed) && parsed["method"] === "tools/call") {
          if (
            !object(params) ||
            params["name"] !== "get_vote" ||
            !object(args) ||
            !Object.values(voteIds).includes(String(args["vote_id"])) ||
            !(typeof parsed["id"] === "string" || typeof parsed["id"] === "number")
          )
            throw new Error("uncorrelated vote tool request");
          route = args["vote_id"] === voteIds.visible ? "visible" : "absent";
          ...
          const outgoing = httpsRequest(
            new URL("/mcp", localOrigin),
            {
              ...
              headers: {
                ...
                ...(route ? { "x-vote-tool-fixture": route } : {})
              },
              ca: tls.cert,
      ...
      client = new Client(
        { name: "vote-tool-native-fixture", version: "1" },
        {
          capabilities: {},
          versionNegotiation: { mode: { pin: "2026-07-28" } },
          cachePartition: "vote-tool-native-fixture"
        }
      );
      ...
      const declared = listed.tools.find((tool) => tool.name === "get_vote");
      ...
      for (const route of ["visible", "absent"] as const) {
        ...
        try {
          ...
          if (!object(text) || text["type"] !== "text" || typeof text["text"] !== "string")
            throw new Error("expected vote tool text result");
          ...
          expect(result["tool"]).toBe("get_vote");
          const data = result["data"];
          if (!object(data)) throw new Error("expected vote result data");
          expect(Object.keys(data)).toEqual(["vote"]);
          expect(data["vote"]).toEqual(oracle[route].view);
          expect(result["reference"]).toBe(voteIds[route]);
          expect(result["resource_uri"]).toBe(resourceUris[route]);
          ...
          observations.push({
            ...
            resourceUri: resourceUris[route],
            reference: voteIds[route],
            ...
            absentProjectionHasNoReservation: route === "absent",
      ...
      releaseSettlement.visible.release();
      releaseSettlement.absent.release();
    ...
    if (failures.length) throw new AggregateError(failures, "native vote tool fixture failed");
    console.info(
      JSON.stringify({
        probe: "native-vote-tool-delivery",
        protocol: "2026-07-28",
        tools: observations,
