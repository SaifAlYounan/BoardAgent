...
import {
  ...
  originalMeetingRows,
  readOriginalMeetingList,
  readOriginalMeetingListMeasurement,
  type OracleMeetingListInput,
  type OracleMeetingListKind,
...
const origin = "https://boardagent.test",
  ...
  token = "synthetic-meeting-lists-native-token";
const timeout = 10000;
const routes = [
  "meetings-full",
  "meetings-page",
...
type Tool = "list_meeting_transcripts" | "list_meetings";
type Selection = {
  tool: Tool;
  kind: OracleMeetingListKind;
  reference: string | null;
  selection: OracleMeetingListInput;
  expectedRows: number;
  input: Record<string, JsonValue>;
  inputCursorAnchor?: OriginalMeetingPageRow;
...
it("delivers exact meeting list pages through native HTTPS SDK and real empty collector settlement", async () => {
  await withMigratedDatabase("meeting_lists_native", async (pool) => {
    const fixture = await seedMeetingToolProjectionFixture(pool),
      actor = fixture.secretary;
    expect(fixture.commands).toHaveLength(6);
    const initialTranscript = await fixture.createTranscript();
    ...
    // Six default normal commands plus initial transcript creation precede reads.
    // One normal replacement occurs before the final read, outside the MCP service.
    // The native service itself rejects every mutation and resource entry point.
    const originalMeetings = await withRequestTransaction(
      pool,
      actor.context,
      async (connection) => {
        expect(
        ...
        const rows = await readOriginalMeetingList(connection, {
          kind: "meetings",
        ...
        expect(new Set(rows.map((row) => row.cursor_id))).toEqual(
          new Set([fixture.meetingId, fixture.emptyMeetingId])
        );
        return rows;
      },
    ...
    for (const phase of ["full", "page", "next", "empty"] as const) {
      const anchor =
        phase === "next"
          ? originalMeetings[0]!
          : phase === "empty"
            ? originalMeetings.at(-1)!
            : null;
      const selection: OracleMeetingListInput = {
        kind: "meetings",
        selectorId: actor.boardId,
        memberId: actor.memberId,
      ...
      if (phase === "empty") {
        // A terminal public page emits no cursor. This is explicitly a synthetic
        // valid signed input anchored to the actual terminal original row.
        const issued = Math.floor(Date.now() / 1000);
        cursor = signMeetingListTailCursor(
          "meetings",
          actor.boardId,
          anchor!,
          actor,
      ...
      selections[`meetings-${phase}`] = {
        kind: "meetings",
        tool: meetingListTools.meetings,
        reference: null,
        selection,
    ...
    for (const phase of ["visible", "empty", "replaced"] as const) {
      const meetingId = phase === "empty" ? fixture.emptyMeetingId : fixture.meetingId;
      selections[`transcript-${phase}`] = {
        kind: "transcripts",
        tool: meetingListTools.transcripts,
        reference: meetingId,
        selection: {
          kind: "transcripts",
    ...
    // Initial and replaced transcript calls intentionally have identical public
    // selectors. Bind each real request to the currently expected sequential route.
    ...
    function routeFor(tool: unknown, input: Record<string, unknown>): Route {
      if (
        ...
        throw new Error("unexpected meeting list selector");
      return activeRoute;
    }
    async function expectedFor(route: Route) {
      ...
      return withRequestTransaction(
        ...
        actor.context,
        async (connection) => {
          const measured = await readOriginalMeetingListMeasurement(connection, choice.selection);
          assertOriginalMeetingFlatScalars(choice.kind, measured);
          const rows = originalMeetingRows(measured);
          expect(rows).toHaveLength(choice.expectedRows);
          if (choice.kind === "transcripts" && rows.length) {
            expect(rows[0]!.item).toMatchObject({
              transcript_id: currentTranscript.transcriptId,
              meeting_id: fixture.meetingId,
              current_version_id: currentTranscript.versionId,
              sha256: currentTranscript.sha256,
              version: route === "transcript-replaced" ? 2 : 1
            });
          }
          const bound = originalMeetingAllocationBound(choice.kind, measured);
    ...
    expect(live.scope_set).toHaveLength(5);
    ...
    const server = createServer(tls, (request, response) => {
      const route = request.headers["x-meeting-lists-fixture"];
      if (isRoute(route)) {
    ...
    // Plain meeting lists return no prepared delivery handle. Delegate
    ...
    try {
      ...
      const service: BoardAgentSurfaceService = {
        ...
        executeRead: async (principal, tool, input) => {
          if ((tool !== "list_meeting_transcripts" && tool !== "list_meetings") || !object(input))
      ...
      const transportFetch: typeof fetch = async (input, init) => {
        ...
        if (object(parsed) && parsed["method"] === "tools/call") {
          if (
            !object(params) ||
            (params["name"] !== "list_meeting_transcripts" && params["name"] !== "list_meetings") ||
            !object(args) ||
          ...
          const outgoing = httpsRequest(
            new URL("/mcp", localOrigin),
            {
              ...
              headers: {
                ...
                ...(route ? { "x-meeting-lists-fixture": route } : {})
              },
              ca: tls.cert,
      ...
      client = new Client(
        { name: "meeting-lists-native-fixture", version: "1" },
        {
          capabilities: {},
          versionNegotiation: { mode: { pin: "2026-07-28" } },
          cachePartition: "meeting-lists-native-fixture"
        }
      );
      ...
      for (const name of ["list_meeting_transcripts", "list_meetings"] as const) {
      ...
      for (const route of routes) {
        if (route === "transcript-replaced") {
          currentTranscript = await fixture.replaceTranscript();
          expect(fixture.commands).toHaveLength(8);
          expect(currentTranscript.transcriptId).toBe(initialTranscript.transcriptId);
          expect(currentTranscript.versionId).not.toBe(initialTranscript.versionId);
          expect(currentTranscript.sha256).not.toBe(initialTranscript.sha256);
        }
        ...
        try {
          ...
          if (route === "meetings-page") {
            const cursor = structured["data"]["next_cursor"];
            expect(typeof cursor).toBe("string");
            selections["meetings-next"].input["cursor"] = cursor as string;
            selections["meetings-next"].cursorIssued = {
          ...
          observations.push({
            ...
            syntheticSignedTailInput: route === "meetings-empty",
    ...
    console.info(
      JSON.stringify({
        probe: "native-meeting-lists-delivery",
        protocol: "2026-07-28",
        normalMeetingCommands: fixture.commands,
        transcriptVersions: [initialTranscript, currentTranscript],
        tools: observations,
