...
export async function seedQuestionProjectionLink(
  pool: Pool,
  actor: AuthorizedActorFixture,
  questionId: string
) {
  ...
  const packageBytes = jsonBytes({
    ...
    fixture: "question link SQL only Δ 🙂",
    voteId: vote,
    packageVersion: 1,
  ...
  const creation = await syntheticEvidence(pool, actor, {
    id: vote,
    type: "vote",
  ...
  const cutoff = await pool.query<{ ordinal: number; text_sha256: Buffer }>(
    "select ordinal,text_sha256 from management_question_turns where question_id=$1 order by ordinal desc limit 1",
    [questionId]
  );
  const last = cutoff.rows[0];
  if (!last) throw new Error("question link fixture has no source turn");
  // The fixed hash/ordinal is a constrained test linkage, not a public cutoff
  // manifest proof. The admission oracle preserves exactly this stored value.
  const linkId = testId(222010);
  await pool.query(
    `insert into question_decision_links(id,organization_id,board_id,question_id,
      inclusive_turn_ordinal,inclusive_turn_sha256,decision_package_id,decision_package_version,
  ...
  return {
    ...
    exclude: async () => {
      ...
      if (inserted.rowCount !== 1) throw new Error("question link recusal was not stored");
    }
  };
}
